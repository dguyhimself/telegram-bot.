...
function SETTINGS_KB() {
  return Markup.inlineKeyboard([
    ...
    [
      Markup.button.callback("💰 Auto-Snipe Buy", "menu_buy_amount"),
    ...
    [
      Markup.button.callback("❔ Confirmations", "menu_confirmations"),
...
bot.action("snipe_auto", async (ctx) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (Math.random() < speedFactor) {
          ...
          // The win/loss probabilities remain the same, but the amounts are changed.
          if (tradeOutcomeRoll < 0.68) {
            // This is a WIN condition (covers "Win" and "Big Win" scenarios, ~65% chance)
            // Generate a random win amount between 100 and 300.
            delta = 100 + Math.random() * 200;
            meta.outcome = "Big Win";
          } else {
            // This is a LOSS condition (covers "Rug Pull" and "Stop-Loss", ~35% chance)
            // Generate a random loss amount between 50 and 70.
            delta = -(50 + Math.random() * 20);
  ...
  }, 5000); // The dashboard will refresh every 2 seconds
  await ctx.answerCbQuery();
});
...
bot.action("menu_controls", async (ctx) => {
  const kb = Markup.inlineKeyboard([
    [Markup.button.callback("💼 View Wallet Details", "menu_view_wallet")],
    [Markup.button.callback("⬅ Back", "menu_main")],
  ]).reply_markup;
...
bot.action("menu_reset", async (ctx) => {
  ...
  if (s.settings.requireConfirmation) {
    const confirmationKeyboard = Markup.inlineKeyboard([
      [
        Markup.button.callback("✅ Yes, Reset Now", "reset_confirm"),
...
async function processReset(ctx, s) {
  ...
  await safeEditOrReply(
    ctx,
    `✅ Session reset. Your license, wallets, and settings have been preserved.`,
    MAIN_KB(),
  );
}
